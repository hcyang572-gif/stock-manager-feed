...
# ── 호가불균형·대량체결(실시간 미시구조) 점수 보너스 (★사용자 지시: "매우 높은" 가중치)
# intraday_refresh 가 5분마다 관심종목·직전 선정 신호 대상에만 실측으로 채우는
# orderbook_imbalance / large_trade_* 필드를 재분석 점수에 반영한다. "매우 높은"이지
# "압도적"(외국인/기관 전환 ±50/38)은 아니므로 그보다 비슷하거나 약간 낮은 스케일
# (대략 ±30~52)로 잡고, **각 보너스에 개별 상한**을 둬 외국인전환+호가불균형+대량체결이
# 한 종목에 다 떠도 총합이 폭주하지 않게 한다. 매도우위·매도방향 대량체결은 '빠른 회피'
# 신호라 가점보다 약간 큰 감점으로 대칭 처리한다. 없음(unavailable)·표본부족·스테일이면
# 스킵(날조 금지). 보너스는 apply_supply_finance 와 동일하게 0~100 클램프 '밖'에서 가산.
_OB_ABS_THRESHOLD = 0.5      # |orderbook_imbalance| 이 이상일 때만 반영(약한 불균형 무시).
_OB_BUY_BASE, _OB_BUY_MAX = 18, 40    # 매수우위: 0.5 에서 +18 → +1.0(전량 매수잔량)서 최대 +40.
_OB_SELL_BASE, _OB_SELL_MAX = 20, 44  # 매도우위: 약간 큰 감점(빠른 회피) -20 → 최대 -44.
_OB_UNSTABLE_FACTOR = 0.5   # stable=False(직전 대비 급변·허수호가 의심)면 보너스 절반으로 신뢰↓.
_OB_STALE_MIN = 10          # orderbook_imbalance_asof 가 이 분(分)보다 오래되면 스킵(신선도 정직).
_LT_BUY_BASE, _LT_BUY_MAX = 26, 46    # 대량체결 매수: 기본 +26 → 배수·체결대금 클수록 최대 +46.
_LT_SELL_BASE, _LT_SELL_MAX = 30, 52  # 대량체결 매도: 대칭이되 약간 큰 감점(빠른 회피).
_LT_STALE_MIN = 10          # large_trade_asof 가 이 분보다 오래되면 스킵(감지 자체는 5분 이내지만 안전 게이트).


def _micro_age_min(asof, now=None):
    """미시구조 필드 asof(ISO 문자열)와 현재의 경과 분(分). 파싱 실패·없음이면 None
    (게이트 미적용 — 과도한 폐기 방지). tz 없으면 KST 로 간주."""
    if not asof:
        return None
    try:
        ts = datetime.datetime.fromisoformat(str(asof))
        if ts.tzinfo is None:
            ts = ts.replace(tzinfo=KST)
        return ((now or datetime.datetime.now(KST)) - ts).total_seconds() / 60.0
    except Exception:
        return None
...
def apply_microstructure(score, why, breakdown, micro, now=None):
    """호가불균형(orderbook_imbalance)·대량체결(large_trade_*)을 '매우 높은' 가중치로
    점수에 반영한다(★사용자 지시, 2026-07-09). intraday_refresh 가 5분마다 실측으로
    채운 필드만 사용하며 없음(unavailable)·표본부족·스테일이면 스킵(날조 금지).
    - 매수우위 호가·매수방향 대량체결 → 강한 가점(강도·배수·체결대금에 비례, 개별 상한).
    - 매도우위 호가·매도방향 대량체결 → 대칭이되 약간 큰 감점(빠른 회피 신호).
    보너스는 apply_supply_finance 와 동일하게 0~100 클램프 '밖'에서 별도 가산해 100
    상한에 무력화되지 않게 하되, 각 항목에 개별 상한을 둬 여러 보너스가 겹쳐도(외국인
    전환+호가+대량체결) 총합이 폭주하지 않게 한다. why/breakdown 에 사람이 읽을 근거와
    신선도(방금/N분 전)를 남긴다. micro 없으면 점수 불변."""
    if not micro:
        return score
    now = now or datetime.datetime.now(KST)
    s = score

    # ── ① 호가 불균형(orderbook_imbalance, -1~+1; +면 매수잔량 우위) ─────────
    imb = micro.get("orderbook_imbalance")
    src = micro.get("orderbook_imbalance_source")
    if imb is not None and src and src != "unavailable":
        age = _micro_age_min(micro.get("orderbook_imbalance_asof"), now)
        if age is not None and age > _OB_STALE_MIN:
            pass  # 스테일(10분 초과) — 스킵(신선도 정직).
        elif abs(imb) >= _OB_ABS_THRESHOLD:
            stable = micro.get("orderbook_imbalance_stable", True)
            # 임계(0.5)에서 base, 극단(±1.0)에서 max 까지 선형 비례.
            frac = min(1.0, (abs(imb) - _OB_ABS_THRESHOLD) / (1.0 - _OB_ABS_THRESHOLD))
            fresh_txt = "방금" if (age is None or age <= 6) else f"{int(age)}분 전"
            unstable_tag = "(직전 대비 급변·허수호가 의심 신뢰↓)" if not stable else ""
            if imb > 0:  # 매수우위 — 강한 가점.
                pts = _OB_BUY_BASE + round(frac * (_OB_BUY_MAX - _OB_BUY_BASE))
                if not stable:
                    pts = round(pts * _OB_UNSTABLE_FACTOR)  # 급변이면 신뢰↓ → 절반.
                pts = min(_OB_BUY_MAX, pts)
                if pts > 0:
                    s += pts
                    why.append(f"호가 매수우위(+{imb * 100:.0f}%)")
                    breakdown.append(
                        f"호가 불균형 강한 매수우위(+{imb * 100:.0f}%, {fresh_txt})"
                        f"{unstable_tag} — 높은 우선 반영 +{pts}")
            else:  # 매도우위 — 약간 큰 감점(빠른 회피).
                pts = _OB_SELL_BASE + round(frac * (_OB_SELL_MAX - _OB_SELL_BASE))
                if not stable:
                    pts = round(pts * _OB_UNSTABLE_FACTOR)
                pts = min(_OB_SELL_MAX, pts)
                if pts > 0:
                    s -= pts
                    why.append(f"호가 매도우위({imb * 100:.0f}%)")
                    breakdown.append(
                        f"호가 불균형 강한 매도우위({imb * 100:.0f}%, {fresh_txt})"
                        f"{unstable_tag} — 빠른 회피 감점 -{pts}")

    # ── ② 대량체결(large_trade_*) — 평소 체결 대비 큰 단일 체결 ─────────────
    if micro.get("large_trade_detected"):
        direction = micro.get("large_trade_direction")
        mult = micro.get("large_trade_multiple") or 0
        val = micro.get("large_trade_value_krw") or 0
        age = _micro_age_min(micro.get("large_trade_asof"), now)
        stale = age is not None and age > _LT_STALE_MIN
        if direction in ("buy", "sell") and not stale:
            # 배수(감지 임계 8배에서 시작)·체결대금이 클수록 비례 가산(각 개별 상한).
            mult_kick = min(14, max(0, round((mult - 8.0) * 1.5)))
            val_kick = min(8, round(val / 1e8 * 2))  # 1억원당 +2, 최대 +8.
            fresh_txt = "방금" if (age is None or age <= 6) else f"{int(age)}분 전"
            val_eok = val / 1e8
            if direction == "buy":
                pts = min(_LT_BUY_MAX, _LT_BUY_BASE + mult_kick + val_kick)
                s += pts
                why.append(f"대량 매수체결({mult:.1f}배)")
                breakdown.append(
                    f"대량체결 감지(평소 대비 {mult:.1f}배, {val_eok:.1f}억원, 매수, "
                    f"{fresh_txt}) — 높은 우선 반영 +{pts}")
            else:  # sell — 대칭이되 약간 큰 감점(빠른 회피).
                pts = min(_LT_SELL_MAX, _LT_SELL_BASE + mult_kick + val_kick)
                s -= pts
                why.append(f"대량 매도체결({mult:.1f}배)")
                breakdown.append(
                    f"대량체결 감지(평소 대비 {mult:.1f}배, {val_eok:.1f}억원, 매도, "
                    f"{fresh_txt}) — 빠른 회피 감점 -{pts}")
...
def score_watchlist(watchlist, regime, cutoff, sf_map=None, weights=None,
                    combo_table=None, micro_map=None):
    ...
    for item in watchlist:
        ...
        sc = apply_microstructure(sc, why, bd, micro_map.get(code))  # 호가불균형·대량체결(★매우 높은 가중치).
        ind["_why"] = why
        ind["_breakdown"] = bd
...
def main():
    ...
    # 직전 feed 의 실시간 미시구조(호가불균형·대량체결) — 재분석 점수 보너스용(★매우 높은
    # 가중치). intraday_refresh 가 5분마다 관심종목·직전 신호 대상에만 실측으로 채워둔 값.
    micro_map = load_prev_microstructure()
    if micro_map:
        print(f"[analyze] 직전 feed 미시구조(호가불균형·대량체결) 로드: {len(micro_map)}종목")

    wl_cand = score_watchlist(watchlist, regime, cutoff, sf_map, weights, combo_table, micro_map)
    mk_cand = score_universe(uni, regime, sf_map, weights, combo_table, micro_map)
